...
def load_dataset(
    ...
    load_frames=True,
    num_pcs=10,
    datapath="moseq_data/saline_example",
    datapath2="amph_data/amphetamine_example",
    mix_data=False,
):
    """
        ...
        for t in trange(len(indices)):
            ...
            with NWBHDF5IO(nwb_path, mode="r") as io:
                ...
                for slc, data in zip([train_slc, test_slc], [train_data, test_data]):
                    data["raw_pcs"] = f.processing["MoSeq"]["PCs"]["pcs_clean"].data[
                        slc
                    ][:, :num_pcs]
                    data["times"] = f.processing["MoSeq"]["PCs"][
                        "pcs_clean"
                    ].timestamps[slc][:]
                    data["centroid_x_px"] = f.processing["MoSeq"]["Scalars"][
                        "centroid_x_px"
                    ].data[slc][:]
                    data["centroid_y_px"] = f.processing["MoSeq"]["Scalars"][
                        "centroid_y_px"
                    ].data[slc][:]
                    data["angles"] = f.processing["MoSeq"]["Scalars"]["angle"].data[
                        slc
                    ][:]
                    data["labels"] = f.processing["MoSeq"]["Labels"][
                        "labels_clean"
                    ].data[slc][:]
                    data["velocity_3d_px"] = f.processing["MoSeq"]["Scalars"][
                        "velocity_3d_px"
                    ].data[slc][:]
                    data["height_ave_mm"] = f.processing["MoSeq"]["Scalars"][
                        "height_ave_mm"
                    ].data[slc][:]

                # only load the frames on the test data
                test_data["frames"] = f.processing["MoSeq"]["Images"]["frames"].data[
                    test_slc
                ]
...
def precompute_ar_covariates(dataset, num_lags=1, fit_intercept=False):
    """
