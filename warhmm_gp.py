...
class TWARHMM_GP(TWARHMM):
    def __init__(self, config, taus, kernel):
        ...
        self.taus = taus
        self.observations = LinearRegressionObservations_GP(
    ...
    def fit(
        ...
        for itr in trange(num_epochs):
            ...
            avg_train_ll = (
                sum([p.marginal_likelihood() for p in posteriors])
                + self.observations.log_prior_likelihood().detach().numpy()
            ) / num_train
            train_lls.append(
                avg_train_ll
            )  # TO DO: need to add prior log likelihood to overall objective function
    ...
    def M_step(
        ...
        if fit_kernel_params:
            self.observations.hyper_M_step(niter=hyper_M_iter, learning_rate=1e-6)

    def compute_expected_suff_stats(
        self, dataset, posteriors, taus, fit_observations=True, fit_transitions=False
    ):
        ...
        def _compute_expected_suff_stats(data, posterior, taus, fit_transitions):
            ...
            if fit_observations:
                xxTw = np.einsum(
                    "tij, tkm -> kijm", xxT, w, optimize="optimal"
                )  # K x D x D x M
        ...
        for data, posterior in zip(dataset, posteriors):
            these_stats = _compute_expected_suff_stats(
                data, posterior, taus, fit_transitions
            )
...
class LinearRegressionObservations_GP(LinearRegressionObservations):
    ...
    def __init__(
        ...
        if random_weights:
            self.weights = np.zeros(
                (num_states, data_dim, covariate_dim, self.num_taus)
            )
            for k in range(num_states):
                for m in range(self.num_taus):
                    self.weights[k, :, :data_dim, m] = scipy.linalg.logm(
        ...
        self.covs = 0.15 * np.tile(np.eye(data_dim), (num_states, 1, 1))
    ...
    def M_step(self, continuous_expectations):
        ...
        (
            ...
            dxdxTw,
            wk,
        ) = continuous_expectations  # LD: modified this to try tensor update
        ...
        Ker = (
            self.weight_gp_kernel(to_t(self.taus)).detach().numpy()
            + kernel_ridge * np.eye(self.num_taus)[None, :, :]
        ...
        for k in range(K):
            ...
            Sigma = J1t + J2t
            QinvdxxTw = np.einsum(
                "pj, jlm -> plm", Qinv[k], dxxTw[k], optimize="optimal"
            )  # D x D x M

            mu = np.linalg.inv(Sigma) @ QinvdxxTw.flatten(
                order="F"
            )  # linear solve might be faster here
            Ahat[k, :, :, :] = mu.reshape(
                Dx, D, M, order="F"
            )  # C style reordering would be faster but using column vector convention for now

            # update covariance
            AxxTwAT = np.einsum(
                "ijm, jlm, plm -> ip", Ahat[k], xxTw[k], Ahat[k], optimize="optimal"
            )
            dxxTATw = np.einsum("jlm, plm -> jp", dxxTw[k], Ahat[k], optimize="optimal")

            Qhat[k, :, :] = (
                AxxTwAT + dxdxTw[k].sum(axis=2) - dxxTATw - dxxTATw.T
            ) / wk[k]
    ...
    def log_likelihoods(self, data):
        ...
        if self.weights.shape[1] == self.weights.shape[2]:
            means = to_t(
                np.einsum(
                    "kijm, tj -> tkmi",
                    self.weights + np.eye(self.weights.shape[1])[None, :, :, None],
                    x,
                    optimize="optimal",
                )
            )
            ...
            means = to_t(
                np.einsum(
                    ...
                    + np.column_stack(
                        (
                            np.eye(self.weights.shape[1]),
                            np.zeros((self.weights.shape[1], 1)),
                        )
                    )[None, :, :, None],
                    x,
                    optimize="optimal",
        ...
        log_likes = torch.distributions.MultivariateNormal(
            means, covs[None, :, None, :, :], validate_args=False
        ).log_prob(
            y[:, None, None, :]
        )  # gives TxKxM log likelihoods
    ...
    def log_prior_likelihood(self):
        ...
        Kcov = (
            self.weight_gp_kernel(tau_grid_torch)
            + kernel_ridge * torch.eye(self.num_taus)[None, :, :]
        )  # add small ridge for stability
        Kinv = torch.inverse(Kcov)

        A_tensor = to_t(
            self.weights
        )  # num_z_states x data_dim x data_dim x num_tau_states
        # pdb.set_trace()

        # \sum_{ijk} -0.5 * a_ijk ' * inv(K_k) * a_ijk
        Kia = torch.matmul(
            Kinv, A_tensor.permute(0, 3, 1, 2).flatten(2, 3)
        )  # now assuming A is K x D x D x M
    ...
    def hyper_M_step(self, niter=100, learning_rate=1e-3):
        ...
        optimizer = torch.optim.SGD(
            self.weight_gp_kernel.parameters(), lr=learning_rate
        )
