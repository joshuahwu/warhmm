...
def plot_discrete_and_continuous_latent_states(
    states_z, states_x, K, colors=None, fig=None, ax=None
):
    ...
    ax.set_yticks(
        np.arange(latent_dim) * lim, ["$x_{}$".format(d + 1) for d in range(latent_dim)]
    )
    ax.set_xticks([])
    ax.set_xlim(0, T)
...
def plot_continuous_states_and_emissions(states_x, emissions):
    ...
    plt.yticks(
        -np.arange(emissions_dim) * lim,
        ["$y_{{ {} }}$".format(n + 1) for n in range(emissions_dim)],
    )
    plt.xlabel("time")
    plt.xlim(0, T)
...
def plot_vector_field(*args, color="black"):
    ...
    ylims = [-2, 2]
    X1, X2 = np.meshgrid(
        np.linspace(xlims[0], xlims[1], 10), np.linspace(ylims[0], ylims[1], 10)
    )
...
def wnb_histogram_plot(
    posteriors,
    tau_duration=False,
    duration_plot=False,
    state_usage_plot=False,
    ordered_state_usage=False,
    state_switch=False,
):
    ...
    total_states = posteriors[0].model.observations.num_states * len(
        posteriors[0].model.observations.taus
    )
    ...
    if tau_duration:
        ...
        fig, ax_array = plt.subplots(
            4,
            int(np.ceil(posteriors[0].model.observations.num_states / 4)),
            sharex=True,
            sharey=True,
        )
        for k in range(posteriors[0].model.observations.num_states):
            ...
            ax.bar(
                np.arange(num_taus),
                duration_means[k * num_taus : k * num_taus + num_taus] * 1000 / 30,
            )
    ...
    if state_switch:
        fig, ax = plt.subplots()
        changepoint_states = np.concatenate(
            [posterior.state_switch() for posterior in posteriors]
        )
...
def plot_multiple_average_pcs(state_list, data_dim, posteriors, spc=4, pad=30):
    """ """
    ...
    for color, state_idx in enumerate(state_list):
        ...
        for d in range(data_dim):
            ...
            if d == data_dim - 1:
                ax.plot(
                    times[:-1],
                    np.abs(np.diff(ex_mean[:, d])) - spc * d,
                    c=cmap(color / len(state_list)),
                    lw=2,
                    label=color,
                )
            else:
        ...
        ax.set_title(
            "Average PCs for State {}".format(((state_idx - num_taus + 1) / num_taus))
        )
...
def centroid_velocity_plot(posteriors):
    ...
    fig, ax_array = plt.subplots(
        4,
        int(np.ceil(posteriors[0].model.num_discrete_states / 4)),
        sharex=True,
        sharey=True,
    )
...
def save_videos_wandb(posteriors):
    for i in trange(posteriors[0].model.num_discrete_states):
        try:
            ...
            wandb.log({filename: wandb.Video(video, fps=30, format="mp4")})
        except:
...
def play(
    movie, fps=30, speedup=1, fig_height=6, filename=None, show_time=False, show=True
):
    ...
    anim = animation.FuncAnimation(
        fig, animate, frames=T // speedup, interval=1, blit=True
    )
    plt.close(anim._fig)
...
def plot_data_and_states(data, states, spc=4, slc=slice(0, 900), title=None):
    ...
    ax.plot(
        times - times[0], x - spc * np.arange(data_dim), ls="-", lw=2, color=palette[0]
    )
...
def extract_syllable_slices(
    ...
    for mouse, posterior in enumerate(posteriors):
        states = np.argmax(
            posterior.expected_states(), axis=1
        )  # //posteriors[0].num_taus
    ...
    for mouse in range(len(posteriors)):
        ...
        slices.append(
            [
                slice(start, start + dur)
                for start, dur in zip(all_starts[is_mouse], all_durations[is_mouse])
            ]
        )
...
def extract_syllable_slices_indiv(
    state_idx,
    posteriors,
    pad=30,
    num_instances=50,
...
def make_crowd_movie(
    ...
    **kwargs
    ...
    crowd_movie = np.zeros(
        (max_dur + pad * 2, raw_size[1], raw_size[0], 3), dtype="uint8"
    )
    ...
    for these_slices, posterior in zip(slices, posteriors):
        ...
        for slc in these_slices:
            ...
            for i in range(lpad + dur + rpad):
                ...
                new_frame_clip = cv2.warpAffine(
                    new_frame_clip.astype("float32"), rot_mat, crop_size
                ).astype(frames.dtype)

                # overlay a circle on the mouse
                if i >= lpad and i <= pad + dur:
                    cv2.circle(new_frame_clip, (xc0, yc0), 3, (255, 0, 0), -1)
                ...
                old_frame[blend_coords] = (
                    0.5 * old_frame[blend_coords] + 0.5 * new_frame[blend_coords]
                )
