...
class TWARHMM(object):
    def __init__(
        self, config, taus=None
    ):  # config is a dictionary containing parameters
        self.config = dict(config)
        self.num_discrete_states = config["num_discrete_states"]
        self.data_dim = config["data_dim"]
        self.covariates_dim = config["covariates_dim"]
        if np.any(taus == None):
            self.taus = np.logspace(
                -config["tau_scale"], config["tau_scale"], config["num_taus"], base=2
            )
        else:
            self.taus = taus
        ...
        self.observations = LinearRegressionObservations(
            self.num_discrete_states,
            self.data_dim,
            self.covariates_dim,
            self.taus,
            config["covariance_reg"],
        )
    ...
    def fit(
        ...
        posteriors = [
            Posterior(self, data_dict, total_states) for data_dict in train_dataset
        ]
        ...
        ) = self.compute_expected_suff_stats(
            train_dataset, posteriors, self.taus, fit_observations, fit_transitions
        )
        train_lls = []
        test_lls = []
        ...
        for itr in trange(num_epochs):
            ...
            avg_train_ll = (
                sum([p.marginal_likelihood() for p in posteriors]) / num_train
            )
            train_lls.append(avg_train_ll)

            # Compute the posteriors for the test dataset too
            test_posteriors = [
                Posterior(self, data_dict, total_states) for data_dict in test_dataset
            ]
            ...
            avg_test_ll = (
                sum([p.marginal_likelihood() for p in test_posteriors]) / num_test
            )
        ...
        for epoch in range(num_epochs):
            ...
            for itr in range(num_batches):
                ...
                self.M_step(
                    ...
                    fit_observations,
                    fit_transitions,
                    fit_tau=fit_tau_trans,
                )

                (
                    these_continuous_expectations,
                    these_discrete_expectations,
                ) = self.compute_expected_suff_stats(
                    minibatch, posteriors, taus, fit_observations, fit_transitions
                )
                ...
                rescaled_cont_stats = tuple(
                    rescale(st) for st in these_continuous_expectations
                )
                rescaled_disc_stats = tuple(
                    rescale(st) for st in these_discrete_expectations
                )
                ...
                continuous_expectations = tuple(
                    sum(x)
                    for x in zip(
                        tuple(st * (1 - stepsize) for st in continuous_expectations),
                ...
                discrete_expectations = tuple(
                    sum(x)
                    for x in zip(
                        tuple(st * (1 - stepsize) for st in discrete_expectations),
                ...
                avg_mll = (
                    sum([p.marginal_likelihood() for p in posteriors]) / this_num_train
                )
            ...
            outer_pbar.update()
            if wandb_log:
                wandb.log({"test_ll": avg_test_mll})
    ...
    def E_step(self, initial_dist, transition_matrix, log_likes, compute_joints=True):
        ...
        alphas, marginal_ll = self.nb_forward_pass(
            initial_dist, transition_matrix, log_likes, max_factor
        )
        ...
        alphas = alphas.reshape(
            (alphas.shape[0], self.num_discrete_states, len(self.taus))
        )
        betas = betas.reshape(
            (betas.shape[0], self.num_discrete_states, len(self.taus))
        )
        log_likes = log_likes.reshape(
            (log_likes.shape[0], self.num_discrete_states, len(self.taus))
        )
        ...
        if compute_joints:  # TODO: split into 2 matrices
            ...
            hadamard_2_z = (
                alphas_z[:-1, :, None]
                * likes_tilde_z[:-1, :, None]
                * likes_tilde_z[1:, None, :]
                * Pz[None, :, :]
                * betas_z[1:, None, :]
            )
            expected_joints_z = hadamard_2_z / np.sum(
                hadamard_2_z, axis=(1, 2), keepdims=True
            )

            hadamard_2_t = (
                alphas_t[:-1, :, None]
                * likes_tilde_t[:-1, :, None]
                * likes_tilde_t[1:, None, :]
                * Pt[None, :, :]
                * betas_t[1:, None, :]
            )
            expected_joints_t = hadamard_2_t / np.sum(
                hadamard_2_t, axis=(1, 2), keepdims=True
            )
    ...
    def M_step(
        ...
        if fit_transitions:
            self.transitions.M_step(discrete_expectations, fit_tau=fit_tau)
        if fit_observations:
            self.observations.M_step(continuous_expectations)
    ...
    def forward_pass(self, initial_dist, transition_matrix, log_likes):
        ...
        for t in range(1, T):
            ...
            alphas[t] = (1 / A_t_minus_1) * np.einsum(
                "ab,bc,cd->ad",
                Pz.T,
                np.reshape(
                    alphas[t - 1] * likes_tilde[t - 1], (Pz.shape[0], Pt.shape[0])
                ),
                Pt,
            ).ravel()
    ...
    def backward_pass(self, transition_matrix, log_likes):
        ...
        for t in range(T - 2, -1, -1):  # iterate from T-2 ==> 0
            ...
            betas[t] = np.einsum(
                ...
                Pz,
                np.reshape(
                    betas[t + 1] * likes_tilde[t + 1], (Pz.shape[0], Pt.shape[0])
                ),
                Pt.T,
            ).ravel()
    ...
    def compute_expected_suff_stats(
        self, dataset, posteriors, taus, fit_observations, fit_transitions
    ):
        ...
        def _compute_expected_suff_stats(
            data, posterior, taus, fit_observations, fit_transitions
        ):
            ...
            (
                fancy_e_z,
                fancy_e_t,
            ) = posterior.expected_transitions()  # TODO: change to return two matrices
            q += 1e-16
            ...
            T = np.zeros(K)
            fancy_e_z_over_T = np.zeros(
                (self.num_discrete_states, self.num_discrete_states)
            )
            ...
            for k in range(K):
                ...
                if fit_observations:
                    ...
                    q_taugivenz = q[:, k, :] / np.sum(
                        q[:, k, :], axis=-1, keepdims=True
                    )
                    E_tau_given_k = np.einsum(
                        "tl,l -> t", q_taugivenz, taus
                    )  # TxL and L -> T
                    E_tauinv_given_k = np.einsum(
                        "tl,l -> t", q_taugivenz, (1 / taus)
                    )  # TxL and L -> T

                    # sufficient stats for A
                    dxxT_Etau[k, :, :] = np.einsum(
                        "t,tij->ij", qzt, data["suff_stats"][1]
                    )
                    xxT[k, :, :] = np.einsum(
                        "t,t,tij->ij", qzt, E_tauinv_given_k, data["suff_stats"][2]
                    )

                    # sufficient stats for Q
                    dxdxT_Etau2[k, :, :] = np.einsum(
                        "t,t,tij->ij", qzt, E_tau_given_k, data["suff_stats"][0]
                    )

                T[k] = np.dot(qzt, np.ones(len(data["data"])))
            ...
            stats = (
                tuple((dxxT_Etau, xxT, dxdxT_Etau2, T)),
        ...
        for data, posterior in zip(dataset, posteriors):
            these_stats = _compute_expected_suff_stats(
                data, posterior, taus, fit_observations, fit_transitions
            )
    ...
    def sample(
        self, T, bias=False
    ):  # TODO: might only work for relatively low total states
        ...
        timescaled_weights, timescaled_covs = self.observations.timescale_weights_covs(
            observations.weights, observations.covs, taus
        )
        if bias:
            x[0, :-1] = MultivariateNormal(
                to_t(np.zeros(observations.data_dim)), to_t(timescaled_covs[z[0], :, :])
            ).sample()
        else:
...
class LinearRegressionObservations(object):
    ...
    def __init__(
        ...
        if random_weights:
            ...
            for i in range(num_states):
                self.weights[i, :, :data_dim] = scipy.linalg.logm(
                    random_rotation(data_dim, theta=np.pi / 20)
                )
        else:
            self.weights = np.zeros((num_states, data_dim, covariate_dim))
        # TODO: do we need this scaling?
        self.covs = 0.05 * np.tile(np.eye(data_dim), (num_states, 1, 1))
    ...
    def precompute_suff_stats(dataset):
        ...
        for i, data in tqdm.tqdm(enumerate(dataset)):
            ...
            if data["data"].shape[1] == data["covariates"].shape[1]:  # no bias
                dx = data["data"] - data["covariates"]
            else:
                dx = data["data"] - data["covariates"][:, :-1]
            data["suff_stats"] = (
                # np.ones(len(data["data"])),
                np.einsum("ti,tj->tij", dx, dx),  # dxn dxn.T
                np.einsum("ti,tj->tij", dx, data["covariates"]),  # dxn xn-1.T
                np.einsum("ti,tj->tij", data["covariates"], data["covariates"]),
            )  # xn-1 xn-1.T
        #
        ###
    ...
    def log_likelihoods(self, data):
        ...
        timescaled_weights, timescaled_covs = self.timescale_weights_covs(
            self.weights, self.covs, taus
        )
        ...
        for k in range(K):
            dist = torch.distributions.MultivariateNormal(
                means[k].T, covs[k], validate_args=False
            )
    ...
    def M_step(self, continuous_expectations):
        ...
        for k in range(self.num_states):
            ...
            self.covs[k] = (
                self.covariance_reg * np.eye(self.data_dim)
                + (
                    dxdxT_Etau2[k]
                    - dxxT_Etau[k] @ AstarT
                    - AstarT.T @ dxxT_Etau[k].T
                    + AstarT.T @ xxT[k] @ AstarT
                )
                / T[k]
            )
    ...
    def timescale_weights_covs(cls, weights, covs, taus):
        ...
        if weights.shape[1] == weights.shape[2]:
            timescaled_weights = (
                np.eye(weights.shape[1]) + tiled_weights / tiled_taus[:, None, None]
            )
        else:
            timescaled_weights = (
                np.hstack((np.eye(weights.shape[1]), np.zeros((weights.shape[1], 1))))
                + tiled_weights / tiled_taus[:, None, None]
            )
...
class Transitions(object):
    def __init__(self, num_discrete_states, num_taus, alpha, kappa, random_init=True):
        ...
        self.initial_dist = np.ones(self.num_discrete_states * self.num_taus) / (
            self.num_discrete_states * self.num_taus
        )
        if random_init:
            Pz = 0.99 * np.eye(self.num_discrete_states) + 0.01 * npr.rand(
                self.num_discrete_states, self.num_discrete_states
            )
            Pz /= Pz.sum(axis=1, keepdims=True)
            Pt = 0.95 * np.eye(self.num_taus) + 0.05 * npr.rand(
                self.num_taus, self.num_taus
            )
            Pt /= Pt.sum(axis=1, keepdims=True)
        else:
            if self.num_discrete_states != 1:
                Pz = 0.99 * np.eye(self.num_discrete_states) + 0.01 / (
                    self.num_discrete_states - 1
                ) * (
                    np.ones((self.num_discrete_states, self.num_discrete_states))
                    - np.eye(self.num_discrete_states)
                )
            else:
                Pz = np.array([[1.0]])
            if self.num_taus != 1:
                Pt = 0.95 * np.eye(self.num_taus) + 0.025 * (
                    np.diag(np.ones(self.num_taus - 1), 1)
                    + np.diag(np.ones(self.num_taus - 1), -1)
                )
                Pt /= Pt.sum(axis=1, keepdims=True)
            else:
    ...
    def M_step(
        self, discrete_expectations, fit_z=True, fit_tau=True
    ):  # TODO: kron first pass is done
        expected_joints_z, expected_joints_t, q_zero = discrete_expectations
        if fit_z:
            expected_joints_z += self.kappa * np.eye(self.num_discrete_states) + (
                self.alpha - 1
            ) * np.ones((self.num_discrete_states, self.num_discrete_states))
            expected_joints_z += 1e-16
            Pz = np.nan_to_num(
                expected_joints_z / expected_joints_z.sum(axis=1, keepdims=True)
            )
        else:
            Pz = self.transition_matrix[0]

        if fit_tau:
            expected_joints_t += self.kappa * np.eye(self.num_taus) + (
                self.alpha - 1
            ) * np.ones((self.num_taus, self.num_taus))
            expected_joints_t += 1e-16
            Pt = np.nan_to_num(
                expected_joints_t / expected_joints_t.sum(axis=1, keepdims=True)
            )
        else:
...
class Posterior(object):
    ...
    def _initialize_posteriors(self, dataset, num_states, seed=0):
        ...
        expected_taus = np.ones(
            (len(dataset["data"]), num_states, 2)
        )  # mu, sigma for each time step and each discrete state
    ...
    def update(self):
        ...
        new_posterior = self.model.E_step(
            self.model.transitions.initial_dist,
            self.model.transitions.transition_matrix,
            log_likes,
            compute_joints=True,
        )
    ...
    def state_durations(states, total_states):
        changepoints = states != np.hstack(
            (states[1:], -1)
        )  # 1 where state change occurs
        ...
        state_durations = np.diff(
            np.hstack((0, changepoint_frame))
        )  # duration before each change
        state_durations[0] += 1
        durations = []
